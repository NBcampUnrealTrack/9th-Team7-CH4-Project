...
public:
	...
	virtual bool Update() override
	{
		...
		{
		case 0:
			if (const auto* EnhancedInput = Cast<UEnhancedInputComponent>(Controller->InputComponent))
			{
				int32 PauseBindings = 0;
				int32 VoiceBindings = 0;
				for (const auto& Binding : EnhancedInput->GetActionEventBindings())
				{
					if (Binding->GetTriggerEvent() != ETriggerEvent::Started) continue;
					if (Binding->GetAction() == Controller->PauseAction) ++PauseBindings;
					if (Binding->GetAction() == Controller->VoiceToggleAction) ++VoiceBindings;
				}
				Test->TestEqual(TEXT("Merged Pause action is bound once"), PauseBindings, 1);
				Test->TestEqual(TEXT("Merged Voice action is bound once"), VoiceBindings, 1);
			}
			else Test->AddError(TEXT("Enhanced Input component is missing"));
		...
		case 8:
			...
			break;
		case 9:
			SendKey(EKeys::P, IE_Pressed);
			break;
		case 10:
			SendKey(EKeys::P, IE_Released);
			break;
		case 11:
			Test->TestTrue(TEXT("dev P-key path opens the same local menu once"), Controller->IsPauseMenuOpen());
			SendKey(EKeys::P, IE_Pressed);
			break;
		case 12:
			SendKey(EKeys::P, IE_Released);
		...
		case 13:
			VerifyClosed();
			Test->AddInfo(TEXT("LOCAL_PAUSE_CHECKS_COMPLETE: single action bindings, ESC, P, Continue, repeat toggle and world tick checked."));
			return true;
		}
		++Stage;
		StageStarted = Now;
